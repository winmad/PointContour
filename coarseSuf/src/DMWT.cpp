...
int DMWT::newTile(int64 wec, float c, int t, int nxt1, int nxt2){
...
__forceinline void DMWT::appendTile(int64 wec, float c, int t, int nxt1, int nxt2, vector<OneTile> &locTileVec){
...
__forceinline int DMWT::appendTile(int64 wec, float c, int t, int nxt1, int nxt2, vector<int> &curTile){
...
int DMWT::computeTriangulation(Boundary<MAXK>& B, Hole& H){
	...
	int64 toAvoidK1,toAvoidK2, toAvoidK;
					...
					for (int h=0; h<numSubHHs; h++){
						...
						for (int tl1=0; tl1<subRes1End-subRes1Start+1; tl1++){
							for (int tl2=0; tl2<subRes2End-subRes2Start+1; tl2++){
								...
								int64 WEK1 = allOneTiles[oti1].WECode;
								float C1   = allOneTiles[oti1].cost;
								int64 WEK2 = allOneTiles[oti2].WECode;
			...
			case 2:
				...
				for (int b=0; b<numSubBBs; b++){
					...
					for (int tl=0; tl<subResEnd-subResStart+1; tl++){
						...
						int64 WWEEK = allOneTiles[oti].WECode;
...
int64 DMWT::GetBitKey(const vector<int>& canItem,const vector<int>& allItem){
	int64 key = 0, mask = 1, one = 1;
...
void DMWT::RecoverListFromBitKey(const vector<int>& list, int64 key,vector<int>& sublist ){
	sublist.clear();
...
void DMWT::loadNormals(vector<int> &numPoints, vector<double*> &inNorms){
