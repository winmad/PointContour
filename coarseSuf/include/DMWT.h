...
#define normal(v,i) normals[v*3+i]
...
//protected:
	...
	int64 GetBitKey(const vector<int>& canItem,const vector<int>& allItem);
	void RecoverListFromBitKey(const vector<int>& list, int64 key,vector<int>& outlist );
	...
	//-------------Tiles
	int newTile(int64 wec, float c, int t, int nxt1, int nxt2);
	int appendTile(int64 wec, float c, int t, int nxt1, int nxt2, vector<int> &curTile);
	void appendTile(int64 wec, float c, int t, int nxt1, int nxt2, vector<OneTile> &locTileVec);
