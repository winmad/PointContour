#ifndef SUFTUNE_MESH_FAIR_H
#define SUFTUNE_MESH_FAIR_H
...
private:
	...
	void JUFairCenter(double ratio, double* oldver, double* newver, double* oldfirstdiffer, double* newfirstdiffer, 
	...
	void JUFairCenter(double ratio12, double ratio3, double* oldver, double* newver, double* oldfirstdiffer, double* newfirstdiffer, 
		double* seconddiffer, vector<int>&vermark, vector<intvector>& verneighbr,HashMap& ver2edgehash, vector<int>&edgelist, vector<double>&wlist, double* norms, double* mags, Volume * grid);
	void computeWeightList(vector<int>&edgelist, vector<double>& wlist);
	void computeDiffer(vector<double>& oldval, vector<double>& differ, vector<double>& wlist, vector<intvector>&verneighbr, HashMap& ver2edgehash);
	void computeDiffer(double* oldval, double* differ, vector<double>& wlist,vector<intvector>&verneighbr, HashMap& ver2edgehash);
	void computeDiffer(double* oldval, double* differ, vector<double>& wlist, vector<int>&vermark,	vector<intvector>&verneighbr, HashMap& ver2edgehash);
	void computeRefreshWeightList(vector<intvector>&verneighbr, vector<double>& w2list);
	void computeNorm(double* oldval, double* norms, double* mags);
...
public:
	...
	void JUFair(double ratio12, double ratio3, int times, Volume * grid);
};
