	...
	const intvector &ctrmedge, const bool &doswap)
{
	interpolate = true;
	doFirstSwap = doswap;
	sufvernum = (int)mver.size()/3;
	suffacenum = (int)mface.size()/3;
...
*/
	...
	sufctredgenum = (int)ctrmedge.size()/2;
	if( sufctredgenum != 0 )
...
void LuMesh::JUFair(double ratio12, double ratio3, int times, Volume * grid)
{
#if (SUFTUNE_OUTPUT)
	cout<<"In JuFair! GOOD!"<<"times:"<<times<<endl;
	cout<<"smoothing....\t";
#endif
	//initialization
	double* oldver;
	...
	{	
		...
		newver = temp;
		temp = NULL;
		JUFairCenter(ratio12, ratio3, oldver, newver, oldfirstdiffer, newfirstdiffer,seconddiffer, 
			vermark, verneighbr, ver2edgehash, edgelist,wlist,norms,mags, grid);
	}	
#if (SUFTUNE_OUTPUT)
	cout<<"done!"<<endl;
#endif
							 ...
							 vector<int>&edgelist)
{
	...
	for( int i = 0; i < edgelen; i ++)
	{
		...
		{
			//cout << "MING: Error! Non-manifold vertices exist! GatherInfo..." << endl;
...
void LuMesh::JUFairCenter(double ratio, double* oldver, double* newver,  double* oldfirstdiffer, double* newfirstdiffer, 
							   ...
							   double* norms, double* mags)
{
	...
	for ( int i = 0 ; i < sufvernum ; i ++ )
	{
		...
		{
			//cout << "MING: Error! Non-manifold vertices exist! JuFairCenter... " << endl;
			// non-manifold
			//dis[3*i] = 0 ;
...
//	int tmove=0;
	for ( int i = 0 ; i < sufvernum ; i ++ )
	{
		...
		{
			//cout << "MING: Error! Non-manifold vertices exist!" << endl;
...
void LuMesh::JUFairCenter(double ratio12, double ratio3, double* oldver, double* newver,  double* oldfirstdiffer, double* newfirstdiffer, 
							   double* seconddiffer, vector<int>&vermark, vector<intvector>& verneighbr,
							   HashMap& ver2edgehash, vector<int>&edgelist, vector<double>&wlist,
							   double* norms, double* mags, Volume * grid)
{
	double* dis = new double[ 3 * sufvernum ] ;
	double* vecs = new double[ 3 * sufvernum ] ;

	double gridEps = grid->getUnit(0) / 10.0;
	//	vector<double> wlist;
	computeWeightList(edgelist, wlist);
	//compute oldfirstdiffer
	computeDiffer(oldver, oldfirstdiffer, wlist, verneighbr, ver2edgehash);
...
//	int tmove=0;
	for ( int i = 0 ; i < sufvernum ; i ++ )
	{
		...
		{
			double curVal = grid->getVolValAtPos(oldver[ 3 * i ], oldver[ 3 * i + 1 ], oldver[ 3 * i + 2 ]);
			// Manifold points: new averaging
			for ( int k = 0 ; k < 3 ; k ++ )
			{
				double migVer [3] = {oldver[ 3 * i ], oldver[ 3 * i + 1 ], oldver[ 3 * i + 2 ]};
				migVer[k] += gridEps;
				double migVal = grid->getVolValAtPos(migVer[0], migVer[1], migVer[2]);
				//newver[ 3 * i + k ] = oldver[ 3 * i + k ] - ratio * seconddiffer[ 3 * i ] * norms[ 3 * i + k ] + ratio * vecs[ 3 * i + k ] ;
				//newver[ 3 * i + k ] = oldver[ 3 * i + k ] - ratio12 * seconddiffer[ 3 * i ] * norms[ 3 * i + k ] + ratio12 * vecs[ 3 * i + k ] - 0.005 * ratio3 * (migVal - curVal) / gridEps;
				newver[ 3 * i + k ] = oldver[ 3 * i + k ] - ratio12 * seconddiffer[ 3 * i ] * norms[ 3 * i + k ] + ratio12 * vecs[ 3 * i + k ] - ratio3 * (migVal - curVal) / gridEps;
			}
...
void LuMesh::computeWeightList(vector<int>&edgelist, vector<double>& wlist)
{
	//inverseedgelenslist
	int edgelen = (int)edgelist.size()/2;
...
void LuMesh::computeDiffer(double* oldval, double* differ, vector<double>& wlist, vector<int>&vermark,
...
void LuMesh::computeDiffer(double* oldval, double* differ, vector<double>& wlist, 
								vector<intvector>&verneighbr, HashMap& ver2edgehash)
{
	memset( differ, 0, sizeof(double)*3*sufvernum);
	for( int i = 0 ; i < sufvernum ; i++)
...
void LuMesh::computeDiffer(vector<double>& oldval, vector<double>& differ, vector<double>& wlist, 
								vector<intvector>&verneighbr, HashMap& ver2edgehash)
{
	for( int i = 0 ; i < sufvernum ; i++)
	{
...
void LuMesh::computeNorm(double* oldval, double* norms, double* mags )
{
...
void LuMesh::computeRefreshWeightList(vector<intvector>&verneighbr, vector<double>& w2list)
{
	// 1/(1 + 1/ni(signma /1nij))
	int* valence = new int[ sufvernum ];
	for( int i = 0; i < sufvernum; i ++)
		valence[ i ] = (int)verneighbr[ i ].size();
	//////////////////////////////////////////////////////////////////////////
	//	for( int i = 0; i < sufvernum; i++)
	//		cout<<valence[i]<<" ";
	//	cout<<endl;
...
*/
	...
	int oldedgenum = (int)edgelist.size()/2;		//before splitting the non-manifold edge, for the splitting those non-contour edges with two contour vertices
	//////////////////////////////////////////////////////////////////////////
#if (SUFTUNE_OUTPUT)
	cout<<"done!"<<endl;
	...
	cout<<"refresh mesh...."<<"\t";
#endif
	sufvernum = (int)verlist.size()/3;
	delete []sufver;
	sufver = new double[ sufvernum*3];
	for( int i = 0; i < sufvernum*3; i++)
		sufver[ i ] = verlist[i];
	suffacenum = (int)facelist.size()/5;
	delete []sufface;
	delete []sufmat;
	delete []suffacenorm;
	sufface = new int[ suffacenum * 3];
						 ...
						 vector<int>&nmedgelist, vector<int>&normedgelist, vector<int>&ctredgelist )
{
	...
	int veri1, veri2;
	int edgenum = (int)edgelist.size()/2;
	bool again = true;
	while( again )
	{
						  ...
						  vector<int>&nmedgelist, vector<int>& normedgelist,vector<int>&facelist, HashMap& ver2edgehash)
{
	...
	int edgelen = (int)edgelist.size()/2;
	int oldnmedgelen = (int)nmedgelist.size();
	...
	for( int j = 0; j < curfacenum; j ++)
	{
		...
		int edgelen2 = (int)edgelist.size()/2;
					   ...
					   vector<int>&nmedgelist, vector<int>& normedgelist,vector<int>&facelist, HashMap& ver2edgehash)
{
	...
	int nmedgelen = (int)nmedgelist.size();
						 ...
						 vector<int>&facelist, double alpha)
{
	...
	oldfacenum = facenum = (int)facelist.size()/5;
