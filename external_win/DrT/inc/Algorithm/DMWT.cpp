...
void DMWT::getResult(double** outFaces,int *outNum, double**outPoints, float** outNorms, int *outPn, bool dosmooth,int subd, int laps)
{
	...
	if (dosmooth){
		...
		*outNum = outputMesh.getFaceCount();
		double *outTris=new double[(*outNum)*9];
		float *newNorms=new float[(*outNum)*9];
		...
		for(int i=0; i<(*outNum); i++){
			for (int j=0;j<3;j++){
				...
				for(int k=0;k<3;k++){
					...
					if(withNormal)
						newNorms[i*9+j*3+k]=(double)pNewNormals[pointID*3+k];
				}
			}
		}
		*outFaces =outTris;
		...
		*outNum = numoftilingtris;
		double *outTris=	new double[numoftilingtris*9];
		for(int i=0; i<numoftilingtris; i++){
			int triId = tiling[i];
			for (int j=0;j<3;j++){
